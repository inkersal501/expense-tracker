  ...
  return (
    <div className={styles.ExpenseList}>
      ...
      <div className={styles.expenseDiv}>
            ...
            <tbody>
              {currentExpenses.map(expense=>(
                <tr key={expense.id} className={styles.expenseTable_row}>
                  <td style={{width:"10%"}}>
                    {expense.category==="Food" && <PiPizza className={styles.catIcon}/>}
                    {expense.category==="Travel" && <BsSuitcase2 className={styles.catIcon}/>}
                    {expense.category==="Entertainment" && <LuGift className={styles.catIcon}/>}
                  </td>
                  <td style={{width:"70%"}}>
                    <span>{expense.title}<br />
                      <span style={{color:"#9d9d9d"}}>{formatDate(expense.date)}</span>
                    </span>
                  </td>
                  <td style={{width:"10%"}}><span style={{color: "#f4bb4a",fontWeight:"bold"}}>₹{expense.amount}</span></td>
                  <td style={{width:"5%"}}><CiCircleRemove className={styles.removeIcon} onClick={()=>handleDelete(expense.id)}/></td>
                  <td style={{width:"5%"}}><MdOutlineModeEdit className={styles.editIcon} onClick={()=>openForm(expense)} /></td>
                </tr>
              ))             
              }            
            </tbody>
        ...
        :
          <h5 style={{color:"#fff"}}>No Transactions Found</h5>
        } 
      <div className={styles.pagination}>
        <button className={styles.navBtn} disabled={currentPage === 1} onClick={handlePrev}><FaArrowLeftLong /></button>
        ...
        <button className={styles.navBtn} disabled={currentPage === totalPages} onClick={handleNext}><FaArrowRightLong/></button>
      </div>
    </div>
      ...
      <div className={styles.modalOverlay}>
        <div className={styles.modalContent}>
          ...
          <form onSubmit={handleForm}>
              ...
              <div style={{display:"flex", gap:"10px", width:"100%", margin:"15px 0"}}> 
                <select id="category" onInput={(e)=>setCategory(e.target.value)} className={styles.input} value={category}>
              ...
              <div style={{display:"flex", gap:"10px", width:"100%", margin:"15px 0"}}>
                <button type='submit' className={styles.addBtn} style={{width:"49%"}}>Update Expense</button>
